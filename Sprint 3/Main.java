...
public class Main {
	...
	public static void main(String[] args) throws IOException, ClassNotFoundException, SQLException {
		...
		while(opt.getOption() != 11) //this is a loop that will keep on going until the quit option is chosen
		{
			...
			if(opt.getOption() == 1)
			{
				...
				Score score = new Score(current, scoreObj.score);
				ScoreDAO scoreDAO = new ScoreDAO("names");
				scoreDAO.save(score);
			}
			...
			if(opt.getOption() == 2)
			{
				...
				String current = currentUser.getCurrentUser();
				Score score = new Score(current, scoreObj.score);
				ScoreDAO scoreDAO = new ScoreDAO("cards");
				scoreDAO.save(score);
			...
			if(opt.getOption() == 4)
			{
				...
				//Training session
				String current = currentUser.getCurrentUser();
				Score score = new Score(current, ng.correctAnswers);
				ScoreDAO scoreDAO = new ScoreDAO("numbers");
				scoreDAO.save(score);
			...
			if (opt.getOption() == 9)
            {
				...
				trs.showTrainingSession(current);
            }
